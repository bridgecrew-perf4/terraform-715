...
resource "aws_route_table" "infra" {
  ...
  tags = merge(
    {
      "Name" = var.single_nat_gateway ? "${var.name}-${var.infra_subnet_suffix}" : format(
        "%s-${var.infra_subnet_suffix}-%s",
        var.name,
        element(var.azs, count.index),
      )
    },
    var.tags,
    local.infra_route_table_tags,
  )
}
...
resource "aws_route_table" "database" {
  count = var.create_vpc && var.create_database_subnet_route_table && length(var.database_subnets) > 0 ? local.nat_gateway_count : 0
...
resource "aws_subnet" "infra" {
  ...
  availability_zone               = length(regexall("^[a-z]{2}-", element(var.azs, count.index))) > 0 ? element(var.azs, count.index) : null
  assign_ipv6_address_on_creation = var.infra_subnet_assign_ipv6_address_on_creation == null ? var.assign_ipv6_address_on_creation : var.infra_subnet_assign_ipv6_address_on_creation

  ipv6_cidr_block = var.enable_ipv6 && length(var.infra_subnet_ipv6_prefixes) > 0 ? cidrsubnet(aws_vpc.this[0].ipv6_cidr_block, 8, var.infra_subnet_ipv6_prefixes[count.index]) : null
...
resource "aws_default_network_acl" "this" {
  ...
  subnet_ids = setsubtract(
    ...
    compact(flatten([
      ...
      aws_network_acl.infra.*.subnet_ids,
...
resource "aws_network_acl" "infra" {
  count = var.create_vpc && var.infra_dedicated_network_acl && length(var.infra_subnets) > 0 ? 1 : 0
  ...
  tags = merge(
    {
      "Name" = format("%s-${var.infra_subnet_suffix}", var.name)
    },
    var.tags,
    var.infra_acl_tags,
  )
...
resource "aws_network_acl_rule" "infra_inbound" {
  count = var.create_vpc && var.infra_dedicated_network_acl && length(var.infra_subnets) > 0 ? length(var.infra_inbound_acl_rules) : 0

  network_acl_id = aws_network_acl.infra[0].id

  egress          = false
  rule_number     = var.infra_inbound_acl_rules[count.index]["rule_number"]
  rule_action     = var.infra_inbound_acl_rules[count.index]["rule_action"]
  from_port       = lookup(var.infra_inbound_acl_rules[count.index], "from_port", null)
  to_port         = lookup(var.infra_inbound_acl_rules[count.index], "to_port", null)
  icmp_code       = lookup(var.infra_inbound_acl_rules[count.index], "icmp_code", null)
  icmp_type       = lookup(var.infra_inbound_acl_rules[count.index], "icmp_type", null)
  protocol        = var.infra_inbound_acl_rules[count.index]["protocol"]
  cidr_block      = lookup(var.infra_inbound_acl_rules[count.index], "cidr_block", null)
}

resource "aws_network_acl_rule" "infra_outbound" {
  count = var.create_vpc && var.infra_dedicated_network_acl && length(var.infra_subnets) > 0 ? length(var.infra_outbound_acl_rules) : 0

  network_acl_id = aws_network_acl.infra[0].id

  egress          = true
  rule_number     = var.infra_outbound_acl_rules[count.index]["rule_number"]
  rule_action     = var.infra_outbound_acl_rules[count.index]["rule_action"]
  from_port       = lookup(var.infra_outbound_acl_rules[count.index], "from_port", null)
  to_port         = lookup(var.infra_outbound_acl_rules[count.index], "to_port", null)
  icmp_code       = lookup(var.infra_outbound_acl_rules[count.index], "icmp_code", null)
  icmp_type       = lookup(var.infra_outbound_acl_rules[count.index], "icmp_type", null)
  protocol        = var.infra_outbound_acl_rules[count.index]["protocol"]
  cidr_block      = lookup(var.infra_outbound_acl_rules[count.index], "cidr_block", null)
}
...
resource "aws_route" "infra_nat_gateway" {
  count = var.create_vpc && var.enable_nat_gateway ? local.nat_gateway_count : 0
...
resource "aws_route" "infra_ipv6_egress" {
  count                       = var.create_vpc && var.create_egress_only_igw && var.enable_ipv6 ? length(var.infra_subnets) : 0
  route_table_id              = element(aws_route_table.infra.*.id, count.index)
  destination_ipv6_cidr_block = "::/0"
  egress_only_gateway_id      = element(aws_egress_only_internet_gateway.this.*.id, 0)
}
...
resource "aws_vpn_gateway_route_propagation" "infra" {
  count = var.create_vpc && var.propagate_infra_route_tables_vgw && (var.enable_vpn_gateway || var.vpn_gateway_id != "") ? length(var.infra_subnets) : 0
